...
;; Version: 4.0
;; Package-Requires: ((s "1.12.0") (dash "2.18.1") (emacs "27.1") (org "9.1") (lf "1.0"))
...
The link text appears as red bold in both Emacs and in HTML export."
 ...
 #+begin_details A parent keymap inherited by o-deflink keymaps
 #+begin_src emacs-lisp
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; We define a parent keymap that o-deflink keymaps inherit from.
...
(defvar org-special-block-extras-mode-map (make-keymap)
  "A keymap of actions, on link types, that is inherited by all `o-deflink' link keymaps.
...
A more Lispy approach would be to use `o-ml'; a macro that produces HTML using Lisp symbols, vectors, and keywords.
For instance, the final `format' above would become:
      (o-ml abbr [:class tooltip :eval (vector :title it)] o-label)
...
We try to get the package's version from a constant “⟨O-LABEL⟩-version” if it exists."
 [:face '(:box "purple")]
 (o-ml a [:eval `[:href ,(concat "https://melpa.org/#/" o-label)]]
       (o-ml img [:eval `[:alt "MELPA"
...
** The doc:o-ml macro                        :ignore:needs_cleaning_up:tests:
   :PROPERTIES:
   :CUSTOM_ID: The-doc-o-ml-macro
   :END:

#+begin_details "o-ml: A ‘M’arkup ‘L’anguage for making HTML using Lisp symbols, vectors, and keywords"
#+begin_src emacs-lisp
(cl-defun o-ml--pair-to-css (vec sep)
...
(4) If the value is a list, then we're likely within a
  ...
  delimiting (parens)."
  (-flatten
   (loop for (key value) on (seq--into-list vec) by #'cddr
        collect (cond ((equal :eval key) (o-ml--pair-to-css (eval value) sep))
                      ((vectorp value) (-->
                                           (loop for (k v) on (seq--into-list value) by #'cddr
                                                 collect (o-ml--pair-to-css (vector k v) ":"))
                                         -flatten
                                         (s-join "; " it)
                                         (o-ml--pair-to-css (vector key it) sep)
                                         (--map (s-replace "\\\"" "" it) it)))
                      (t (format "%s%s%s" (s-chop-prefix ":" (symbol-name key)) sep
                                 (cond ((stringp value) (pp-to-string value))
                                       ((listp value) (o--pp-list value))
                                       (t value))))))))

(defmacro o-ml (tag &optional attributes &rest content)
...
and we use a local variable for the inner-most content.
    ...
    (let ((centred? t)     ;; For use in the [attributes] (2ⁿᵈ) argument
          ...
          (o-ml div [:title \"A form of greeting\"
                     :style [:padding 1em
...
See the definition of the `o-defblock' “box” for a working example.
...
are then catenated to produce a single string."
  `(cl-destructuring-bind (attributes _ content)
     ...
     (format "<%s %s> %s </%s>"
             (quote ,tag)
             (s-join " " (o-ml--pair-to-css attributes "="))
...
"
 ...
 #+RESULTS:
 | :export | (lambda (label description backend) (s-replace-all `((@@ . )) (o--org-demo backend (or description label) label :o-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
"
 ...
 #+RESULTS:
 | :face | (:foreground red :weight bold) | :export | (lambda (label description backend) (s-replace-all `((@@ . )) (o--remark backend (or description label) label :o-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
#+end_center
 ...
 #+begin_details Implementation
 #+begin_src emacs-lisp
(o-defblock box (title "") (background-color nil shadow nil)
...
In the future, I will likely expose more arguments."
  ...
  (pcase backend
    ...
    (_ (-let [haze (lambda (left right deep-right deep-left)
                     `[:box-shadow ,(thread-last (list (cons right      "8px 6px 13px 8px %s")
         ...
         (o-ml div [:style
                    [:padding 1em
                     :eval (vector :background-color (o-subtle-colors (format "%s" (or background-color "green"))))
                     :border-radius 15px
                     :font-size 0.9em
                     :eval (when shadow
                             (cond
                              ...
                              ((or (stringp shadow) (symbolp shadow))
                               `[:box-shadow (10px 10px 20px 0px ,(pp-to-string shadow)) :width 50% :margin auto])
                              ((json-plist-p shadow)
...
:deep-right orange)~ …
 ...
 For further header options, see the documentation of [[doc:o--box][box]].
...
#+RESULTS:
| :export | (lambda (label description backend) (s-replace-all `((@@ . )) (o--calc backend (or description label) label :o-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
* COMMENT Issue ♯8 :Useful_code_to_migrate_to_core:Or_not:
  :PROPERTIES:
  :CUSTOM_ID: TODO-HERE
  :END:
   ...
   #+begin_details ‘defblock’ Implementation
    ...
    "
      ...
      (let* ((optionals              (infer-optionals experimental&&docstring&&body))
             (o-respect-newlines? (cl-first optionals))
    ...
    (cl-defun o-defblock---org-export (backend o-link? o-respect-newlines?)
      "Wrap the given X in an export block for the current backend.
    ...
    o--support-special-blocks-with-args."
      `(lambda (x)
        (if ,o-link?
            x
          ;; o-respect-newlines? is super experimental: It's a bit ugly on the LaTeX side.
          (cond ((and ,o-respect-newlines? (member (quote ,backend) '(html reveal)))
    ...
    (cl-defun o-defblock---org-parse
        (backend o-link? o-respect-newlines?)
      "This should ONLY be called within an ORG-EXPORT call.
    ...
    BODY, list: Code to be executed"
      `(cl-defun ,(intern (format "o--%s" name))
         ...
         (cl-letf (((symbol-function 'org-export)
                    (o-defblock---org-export backend o-link? ,o-respect-newlines?))
                   ((symbol-function 'org-parse)
                    (o-defblock---org-parse backend o-link? ,o-respect-newlines?)))
...
(ert-deftest org-export/spec ()
  ...
  (should (equal "x"
                 (funcall (o-defblock---org-export 'html :o-link :respect-newlines) "x")))

  (should (equal "x"
                 (funcall (o-defblock---org-export 'html :o-link nil) "x")))
...
(ert-deftest org-parse/spec ()
  ...
  (should (equal "x"
                 (funcall (o-defblock---org-parse 'html :o-link :respect-newlines) "x")))

  (should (equal "x"
                 (funcall (o-defblock---org-parse 'html :o-link nil) "x")))
