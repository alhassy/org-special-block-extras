...
TODO relies on an unpublished version of lf.el ?
...
+ Textual expansions :: =show:user-full-name= expands, upon export, to /Musa
  Al-hassy/ on my machine. (The =show= link is defined below.)
...
+ Definitions / Personal Glossary :: ~doc:thread-first, doc:family, doc:Hussain~
  ...
  documentation for the phrase doc:Hussain. (The =doc= link is defined below.)

  # (o-docs-load-libraries (list "~/org-special-block-extras/documentation.org"))
#+end_box

Just as there is doc:defun for “def”ining “fun”ctions and doc:defmacro for
“def”ining “mac”ros, there is also doc:o-deflink for “def”ining “link”s for
...
defined using doc:org-link-set-parameters ---which, unlike doc:o-deflink, is not
...
*** Define links as you define functions: doc:o-deflink
    :PROPERTIES:
    :CUSTOM_ID: NEW-o-deflink
    :END:
 ...
 the input arguments are fixed, the ~o-deflink~ interface is similar to ~cl-defun~ in
...
The link text appears as red bold in both Emacs and in HTML export."
 ...
 #+begin_details Implementation of o-deflink
    ...
    (let ((o-link/NAME (intern (format "o-link/%s" name)))
      ...
      `(progn
       ...
       (cl-defun ,o-link/NAME ;; function name
           ...
           (concat (if ,docstring (format "\n\n%s\n" ,docstring) "\n")
                   ,navigation
                   "\nEXPORT IMPLEMENTATION:\n\n"
                   (pp-to-string (quote (let ,lets ,body))))
           ;; function body
           (let* ,lets ,@body))
       ...
       (org-link-set-parameters
        ...
        :follow (lambda (o-label o-prefix) (let (o-description) (let* ,lets ,(getf display :follow))))
         ;; These links should *never* be folded in descriptive display;
        ;; i.e., “[[example:lable][description]]” will always appear verbatim
        ;; and not hide the first pair […].
        :display (quote ,(getf display :display)) ;; e.g.,: 'full
        ;; Any special keybindings when cursour is on this link type?
        ...
        :keymap (let ((o-keymap (copy-keymap org-mouse-map))
                      ...
                      (msg  (concat
                             ...
                             (pp-to-string (quote ,(getf display :keymap))))))
                  ;; Populate the keymap
                  (loop for (key action)
                       ...
                       (-let* (((&plist :path :format :contents-begin :contents-end)
                               ...
                               (o-description
                                (when (equal format 'bracket)
                                  (copy-region-as-kill contents-begin contents-end)
                                  (substring-no-properties (car kill-ring)))))
                         (or (let* ,lets ,(getf display :help-echo))
                             (format "%s:%s\n\n%s\nHTML Export:\n\n%s"
                                     (quote ,name)
...
* TODO Nice Keystroke Renditions: kbd:C-h_h             :outdated_screenshot:
  :PROPERTIES:
  :CUSTOM_ID:
  :END:
...
- For instance, =[[kbd:key sequence][description]]= will show as [[kbd:key
  sequence][description]]; i.e., the key sequence in nice key font along with a
  tooltip explaining it.

  Whereas, ~kbd:𝒳~ will show a tooltip defining 𝒳, as an Emacs Lisp function, if
  possible.  For example, ~kbd:C-h_h~ is kbd:C-h_h; and likewise =<kbd:M-s h .>= is
  <kbd:M-s h .> (we need to use ~<...>~ since punctuation is not picked up as part
  of link labels).
  ...
  `(o-deflink ,(intern name)
    [:display 'full
     :follow (--> (s-split "|" path)
               ...
               (browse-url it))
    :help-echo
      (-let [ (key value color url logo)  (s-split "|" o-label) ]
        (lf-string "${o-label}
...
+ doc:o--docs is the global variable that holds the glossary/database for the
  current session. Entries may be inserted with doc:o-docs-set and retrieved
  with doc:o-docs-get.

+ doc:o-docs-fallback is a /fallback method/ for retriving documentation-glossary
  ...
  English words. We use doc:o-docs-load-libraries to load documentation-glossary
  entries from external files.

+ doc:o-docs-insert for interactively inserting links for documented-glossary words.
#+end_box
...
That is what we accomplish with this new `show' link type."
  [:face (:underline "green")
   :let (o-value (if (equal o-label "GLOSSARY")
                   ...
                   (pp-to-string (eval (read o-label))))
  ...
  :help-echo (format
              ...
              o-value
              o-expr)]
 (cond ((not (equal o-label "GLOSSARY")) (pp-to-string (eval (read o-label))))

       ;; Otherwise O-LABEL is glossary, which we do not print in HTML
