...
* TODO A unified interface for special blocks and links: ~defblock~ :something_here_is_ruining_my_kbd_renditions:
  :PROPERTIES:
...
+ Textual expansions :: =show:user-full-name= expands, upon export, to /Musa
  ...
  article. Similarly, ~octoicon:home~ becomes octoicon:home; doc:o-link/octoicon.
...
#+end_parallel
 ...
 #+end_org-demo
#+latex: \fi
** TODO COMMENT Practice Problems: /Now you try!/                  :HERE_kbd_is_ruined:
   :PROPERTIES:
...
Examples:
  ...
  (if (equal o-backend 'latex)
    ...
    (if tooltip?
        ...
        (format "<abbr class=\"tooltip\" style=\"border: none; text-decoration: none;\" title=\"%s\">%s</abbr>"
                (o-html-export-preserving-whitespace tooltip)
                keystrokes)
      keystrokes)))
#+end_src
...
#+end_src
#+end_details
* TODO Emacs faces for links                                   :Maybe_delete:
  :PROPERTIES:
