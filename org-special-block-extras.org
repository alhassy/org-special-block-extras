...
# (progn (load-file "org-special-block-extras.el") (org-special-block-extras-mode) (setq org-special-block-extras--docs-libraries  '("~/org-special-block-extras/documentation.org")) (ignore-errors (org-special-block-extras-docs-load-libraries)))
#
...
(org-link-set-parameters
 ...
 : org-special-block-extras--name&doc
...
# (fmakunbound 'org-special-block-extras--org-parse)
...
(add-hook 'org-export-before-parsing-hook 'org-special-block-extras--support-special-blocks-with-args)
 ...
 #+RESULTS:
 | org-special-block-extras--support-special-blocks-with-args | org-ref-acronyms-before-parsing | org-ref-glossary-before-parsing |
...
;; Enable short names; e.g., ‘defblock’ instead of ‘org-special-block-extras-defblock’.
(org-special-block-extras-short-names)
...
"
 ...
 #+RESULTS:
 : org-special-block-extras--blockcall
...
"
 ...
 #+RESULTS:
 | :export | (lambda (label description backend) (s-replace-all `((@@ . )) (org-special-block-extras--org-demo backend (or description label) label :ospe-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
#+RESULTS:
| :export | (lambda (label description backend) (s-replace-all `((@@ . )) (org-special-block-extras--spoiler backend (or description label) label :ospe-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
*** Judgements: Inference rules and proof trees
 ...
 Using a mixture of ~\frac~ and ~\displaystyle~, define a block [[doc:org-special-block-extras--tree][tree]] so that the
...
"
 ...
 #+RESULTS:
 | :face | (:foreground red :weight bold) | :export | (lambda (label description backend) (s-replace-all `((@@ . )) (org-special-block-extras--remark backend (or description label) label :ospe-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
Let's have some sanity tests...
#+begin_src emacs-lisp :tangle old_tests.el :comments link
...
* TODO Folded Details ---As well as boxed text and subtle colours
  :PROPERTIES:
...
* TODO Nice Keystroke Renditions: kbd:C-h_h
  :PROPERTIES:
  :CUSTOM_ID:
  :END:
...
The use of ~doc:𝒳~ --- and [[doc:org-special-block-extras--documentation][documentation]] --- are for explanatory remarks, personal
...
additional explanation; for that we provide [[doc:org-special-block-extras--margin][margin]]. For example:
#+begin_quote
...
use *[[green:org-lists as equational proofs]]* ---thanks to the [[doc:org-special-block-extras--calc][calc]] block!
#+begin_details Implementation :title-color pink
  #+begin_src emacs-lisp
...
#+RESULTS:
| :export | (lambda (label description backend) (s-replace-all `((@@ . )) (org-special-block-extras--calc backend (or description label) label :ospe-link? t))) | :help-echo | (lambda (window object position) (save-excursion (goto-char position) (-let* (((&plist :path :format :raw-link :contents-begin :contents-end) (cadr (org-element-context))) (description (when (equal format 'bracket) (copy-region-as-kill contents-begin contents-end) (substring-no-properties (car kill-ring))))) (format %s |
...
| Marginal remarks       | [[doc:org-special-block-extras--margin][margin]]                      |                     |
| Badges                 | [[doc:org-special-block-extras-make-badge][badge]]                       |                     |
| Equational proofs      | [[doc:org-special-block-extras--calc][calc]]                        |                     |

Other fun stuff: [[doc:org-special-block-extras--solution][solution]], [[doc:org-special-block-extras--org-demo][org-demo]], [[doc:org-special-block-extras--stutter][stutter]], [[doc:org-special-block-extras--rename][rename]], [[doc:org-special-block-extras--spoiler][spoiler]], [[doc:org-special-block-extras--tree][tree]] :grin:
...
(defun org-special-block-extras--org-parse (x)
  "This should ONLY be called within an ORG-EXPORT call."
   ...
   #+begin_details ‘defblock’ Implementation
    ...
    (cl-defun org-special-block-extras-defblock---org-export (backend ospe-link? ospe-respect-newlines?)
      "Wrap the given X in an export block for the current backend.
    ...
    (cl-defun org-special-block-extras-defblock---org-parse
    ...
    BODY, list: Code to be executed"
      `(cl-defun ,(intern (format "org-special-block-extras--%s" name))
         ...
         (cl-letf (((symbol-function 'org-export)
                    (org-special-block-extras-defblock---org-export backend ospe-link? ,ospe-respect-newlines?))
                   ((symbol-function 'org-parse)
                    (org-special-block-extras-defblock---org-parse backend ospe-link? ,ospe-respect-newlines?)))
...
(ert-deftest org-export/spec ()
  ...
  (should (equal "x"
                 (funcall (org-special-block-extras-defblock---org-export 'html :ospe-link :respect-newlines) "x")))

  (should (equal "x"
                 (funcall (org-special-block-extras-defblock---org-export 'html :ospe-link nil) "x")))

  (should (equal "@@html:x@@"
                 (funcall (org-special-block-extras-defblock---org-export 'html nil :respect-newlines) "x")))

  (should (equal (unindent
                   "#+begin_export html
                   x
                   ,#+end_export")
                 (funcall (org-special-block-extras-defblock---org-export 'html nil nil) "x"))))


(ert-deftest org-parse/spec ()

  (should (equal "x"
                 (funcall (org-special-block-extras-defblock---org-parse 'html :ospe-link :respect-newlines) "x")))

  (should (equal "x"
                 (funcall (org-special-block-extras-defblock---org-parse 'html :ospe-link nil) "x")))

  (should (equal "@@x@@html:"
                 (funcall (org-special-block-extras-defblock---org-parse 'html nil :respect-newlines) "x")))
...
,#+begin_export html
"
                 (funcall (org-special-block-extras-defblock---org-parse 'html nil nil) "x"))))

;; MA: TODO: Test org-special-block-extras--support-special-blocks-with-args (⟰)
...
(deftest "Constant blocks export to HTML preserves indentation/enumeration"
  (-let [org-special-block-extras--supported-blocks '(hello)]
    (should (equal
      (⟰ "1. item one
          2. item two
             ,#+begin_hello
