...
* DONE COMMENT Folded Details ---Let the user see stuff only if they're interested
  :PROPERTIES:
...
* DONE COMMENT   ~latex-definitions~ for hiding LaTeX declarations in HTML
   :PROPERTIES:
 ...
 #+end_org-demo
  ...
  #+begin_src emacs-lisp :title "‘latex-definitions’ Implementation" :folded t
...
#+HTML:
:End:
* DONE COMMENT Parallel ---/Place ideas side-by-side, possibly with a separator/
  :PROPERTIES:
  :CUSTOM_ID: Parallel
  :END:
