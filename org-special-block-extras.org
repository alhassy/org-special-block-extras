...
Three example uses:
  ...
  (cl-destructuring-bind (link-display docstring body)
      (lf-extract-optionals-from-rest link-display #'vectorp
                                      ...
                                      body)
    `(list
      ,(org--create-defmethod-of-defblock2 name docstring (plist-get kwds :backend) kwds body)
...
(cl-defmethod org--create-defmethod-of-defblock2 ((name symbol) docstring backend-type (kwds list) (body list))
