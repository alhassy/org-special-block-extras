...
** Define links as you define functions: ~o-deflink~
   :PROPERTIES:
...
** COMMENT Modularity with ~thread-blockcall~
   :PROPERTIES:
...
That is what we accomplish with this new `show' link type."
 [:face (:underline "green")
  :help-echo (format
              ...
              (if (equal o-label "GLOSSARY")
                  (format "%s ---i.e., o--docs-actually-used" o-label)
                o-label))]
...
(advice-add #'org-export-dispatch
  :after (lambda (&rest _)
  (setq o--docs-actually-used nil
        o--docs nil)))
#+end_src
# ‼ TODO MA: I don't usually want to evaluate this block?
