...
;; TODO: Follow “Emacs Lisp Coding Conventions”, e.g., use “org-block--𝒮” not
;; “org-block/𝒮”! Why? Because the defun is an internal implementation detail
;; and so its name should reflect that intent.
...
(load-file “~/snap/snap.el”) ;; For `snap-define-fixture’, `snap-define-relation’, etc.
...
;; `deftest’ is the identity fixture — no wrapping, readable ERT names.
;; snap.el no longer ships it directly; we define it here via snap-define-fixture.
...
;;; org-defblock-only -- terse syntax
;; (ert '(tag "org-defblock-only"))
...
;;; org-defblock-only -- cl-defun-style
;; (ert '(tag "cl-defun-style"))

(cl-defun signature (f)
  "Get signature of method F, a symbol."
  (car (last (s-split "\n" (cl-fifth (seq--into-list (symbol-function f)))))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `nil' args"
  (using (org-defblock-only folded () (format "<details>%s</details>" contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &optional MAIN-ARG &rest ## &key (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `&optional' main arg"
  (using (org-defblock-only folded (&optional title) (format "<details>%s</details>" contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &optional title &rest ## &key (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `&optional' main arg with default value"
  (using (org-defblock-only folded (&optional (title "Click to see more!"))
           (format "<details><summary>%s</summary>%s</details>" title contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &optional (title \"Click to see more!\") &rest ## &key (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `&key' keyword args"
  (using (org-defblock-only folded (&key style class)
           (format "<details style=\"%s\" class=\"%s\">%s</details>" style class contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &rest ## &key STYLE CLASS (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `&key' keyword args with default values₁"
  (using (org-defblock-only folded (&key (style "color:blue") class)
           (format "<details style=\"%s\" class=\"%s\">%s</details>" style class contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &rest ## &key (STYLE \"color:blue\") CLASS (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `&key' keyword args with default values₃"
  (using (org-defblock-only folded (&key style (class "note"))
           (format "<details style=\"%s\" class=\"%s\">%s</details>" style class contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &rest ## &key STYLE (CLASS \"note\") (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))


(deftest "cl-defun-style invocation of `org-defblock-only' may have `&key' keyword args with default values₃"
  (using (org-defblock-only folded (&key (style "color:blue") (class "note"))
           (format "<details style=\"%s\" class=\"%s\">%s</details>" style class contents))
    (should (equal (signature 'org-block/folded)
                   "(fn BACKEND RAW-CONTENTS &rest ## &key (STYLE \"color:blue\") (CLASS \"note\") (CONTENTS-OCCUR-AS-LINK-DESCRIPTION nil) &allow-other-keys)"))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;


(setq args-list '(&optional c))
(--partition-by (member it '(&optional &key)) args-list)


(cl-defun parse-args (args-list)
  (cond
   ((-contains? args-list '&optional)
    (-let [((backend-name contents-name) (main-arg . more-args)) (-split-on '&optional args-list)]
      (list :backend-name  backend-name
            :contents-name contents-name
            :raw-contents-name (intern (format "raw-%s" contents-name))
            :main-arg-name (if (listp main-arg) (car main-arg) main-arg)
            :main-arg-value (if (listp main-arg) (cadr main-arg) nil)
            :keywords (when more-args
                        (if (equal '&key (car more-args))
                            (cdr more-args)
                          (error "“org-defblock[-only]” expected to see “&key” but saw “%s”" (car more-args)))))))
   (:otherwise
    (list :backend-name  'backend
          :contents-name 'contents
          :raw-contents-name 'raw-contents
          :main-arg-name (caar args-list)
          :main-arg-value (cadar args-list)
          :keywords (cdr args-list)))))

;; (ert '(tag "parse-args"))


(deftest "`parse-args' works as intended in terse style"
  (should (equal (parse-args '((title "Details") (style nil) (class nil)))
                 '( :backend-name backend
                    :contents-name contents
                    :raw-contents-name raw-contents
                    :main-arg-name title
                    :main-arg-value "Details"
                    :keywords ((style nil) (class nil))))))

(deftest "`parse-args' works as intended when there is an `&optional' clause"
  (should (equal (parse-args '(a b &optional c &key d))
                 '( :backend-name a
                    :contents-name b
                    :raw-contents-name raw-b
                    :main-arg-name c
                    :main-arg-value nil
                    :keywords (d)))))

(deftest "`parse-args' works as intended when there is an `&optional' clause and no `&key' clause"
  (should (equal (parse-args '(a b &optional c))
                 '( :backend-name a
                    :contents-name b
                    :raw-contents-name raw-b
                    :main-arg-name c
...
(deftest "`parse-args' works as intended when there is an `&optional' clause with main-arg default value"
  (should (equal (parse-args '(a b &optional (c c-default) &key d))
                 '( :backend-name a
                    :contents-name b
                    :raw-contents-name raw-b
                    :main-arg-name c
                    :main-arg-value c-default
                    :keywords (d)))))

(deftest "`parse-args' works as intended when there is an `&optional' clause with main-arg default value and no `&key' clause"
  (should (equal (parse-args '(a b &optional (c c-default)))
                 '( :backend-name a
                    :contents-name b
                    :raw-contents-name raw-b
                    :main-arg-name c
...
;; TODO: WORKING HERE: Try to make this pass 😄
(deftest "`parse-args' works as intended when there is only an `&optional' clause"
  (should (equal (parse-args '(&optional (c c-default)))
                 '( :backend-name backend
                    :contents-name contents
                    :raw-contents-name raw-contents
                    :main-arg-name c
...
;;; define-org-export-test — relational test fixture for org-export assertions
;;
;; Generated by `define-relation'.  Each test is a single export
;; assertion with `C-u C-x C-e' snapshot updating, [Show Diff] and
;; [Update Snapshot] buttons on failure.
;;
;; Tests with multiple `exporting' calls or complex control flow
;; remain as plain `deftest' forms using the `exporting' macro above.

(define-relation org-export (input to using equals modulo)
  "Export INPUT via backend TO, assert result EQUALS.
USING is a list of (org-defblock ...) forms installed temporarily.
MODULO enables regex-based approximate matching."
  (let* ((defs (cond
                ((null using) nil)
                ;; Single (org-defblock ...) form → wrap in list.
                ((and (consp using) (memq (car using) '(org-defblock org-defblock-only)))
                 (list using))
                (t using)))
         (undefs (mapcar (lambda (f) (cons 'org-undefblock (cdr f))) defs))
         (actual (unwind-protect
                     (progn
                       (dolist (f defs) (eval f))
                       (org-export-string (lf-string input) (or to 'html)))
                   (dolist (u undefs)
                     (ignore-errors (eval u))))))
    (if modulo
        (let* ((modulos (if (listp modulo) modulo (list modulo)))
               (pattern (thread-last equals
                          lf-string
                          regexp-quote
                          (s-replace-all (--map (cons it ".*") modulos))
                          (format "^%s$"))))
          (should (equal 0 (string-match-p pattern actual))))
      (should (equal actual (lf-string equals))))
    (list :equals actual)))
...
(define-org-export-test “main-arg may be a quoted string” [header-args]
  :input “#+begin_shout \”Hello, to the \”
          world
          #+end_shout”
...
(define-org-export-test "main-arg may be omitted" [header-args]
  :input "#+begin_shout
          world
          #+end_shout"
  :using (org-defblock shout (greeting) "docs" (concat greeting (upcase contents)))
...
(define-org-export-test "main-arg may be omitted but keywords provided" [header-args]
  :input "
          #+begin_shout :to me
          world
          #+end_shout
...
(define-org-export-test "`org-defblock' exports a custom block to HTML with content formatting"
  :input "Look:
          #+begin_highlight Warning :style color:orange
          Something **important** here.
          #+end_highlight"
...
(define-org-export-test "indented blocks preserve list structure in HTML output" [issue♯8]
  :input "- item one
          - item two
            #+begin_testblock foo
            inner
...
(define-org-export-test "indented blocks preserve list structure in LaTeX export" [issue♯8]
  :input "1. First
          2. Second
             #+begin_testblock foo
             inner
...
(define-org-export-test "indented blocks, in enumerations, do not introduce extra blank lines" [issue♯8]
  :input "- A
          #+begin_testblock foo
          body
          #+end_testblock
...
(define-org-export-test "enumerations are preserved when they contain special blocks, for LaTeX" [issue♯8]
  :input "
...
(define-org-export-test "enumerations are preserved when they contain special blocks, for HTML" [issue♯8]
  :input "
          1. builtin source block
             #+begin_src latex
             \\uline{content 1}
...
(define-org-export-test "enumerations are preserved for blocks inside lists inside blocks" [issue♯8]
  :input "
          #+begin_foo X
          1. Something\\
             Indented line no. 1
